...
def _dir_has_files(path, *names):
    if not path or not os.path.isdir(path):
        return False
    return all(os.path.isfile(os.path.join(path, name)) for name in names)
...
def _ensure_tcl_paths():
    """Не перебивать рабочие пути PyInstaller; чинить только сломанные."""
    if not getattr(sys, "frozen", False):
        return

    tcl_now = os.environ.get("TCL_LIBRARY")
    tk_now = os.environ.get("TK_LIBRARY")
    tcl_ok = _dir_has_files(tcl_now, "init.tcl", "auto.tcl")
    tk_ok = _dir_has_files(tk_now, "tk.tcl")
    if tcl_ok and tk_ok:
        return
    ...
    if not tcl_ok:
        tcl = (
            _find_lib_dir(os.path.join(base, "_tcl_data"), ("init.tcl", "auto.tcl"))
            or _find_lib_dir(base, ("init.tcl", "auto.tcl"))
        )
        if tcl:
    ...
    if not tk_ok:
        tk = (
            _find_lib_dir(os.path.join(base, "_tk_data"), ("tk.tcl",))
            or _find_lib_dir(base, ("tk.tcl",))
        )
        if tk:
            os.environ["TK_LIBRARY"] = tk


def _downloaded_script_path():
    if os.name == "nt":
    ...
    return os.path.join(base, "ProductManager", "app_latest.py")


def _handoff_to_downloaded_script():
    """Запускает скачанный .py до import tkinter, чтобы Tcl не инициализировался дважды."""
    ...
    if __name__ != "__main__":
        return False
    try:
        if os.path.basename(os.path.abspath(__file__)) == "app_latest.py":
            return False
    except NameError:
        pass
    updated = _downloaded_script_path()
